...
# sum_stat$ZIPcov_aic <- ZIPcov_info$aic
# sum_stat$ZIPcov_de <- ZIPcov_info$de
# sum_stat$ZIPcov_exp_zerofrac <- ZIPcov_info$pi+(1-ZIPcov_info$pi)*exp(-ZIPcov_info$mu)
# sum_stat$ZIPcov_mean <- (1-ZIPcov_info$pi)*ZIPcov_info$mu
# sum_stat$ZIPcov_var <- (1-ZIPcov_info$pi)*ZIPcov_info$mu*(1+ZIPcov_info$mu*ZIPcov_info$pi)
...
# sum_stat$ZINBcov_aic <- ZINBcov_info$aic
# sum_stat$ZINBcov_de <- ZINBcov_info$de
# sum_stat$ZINBcov_exp_zerofrac <- ZINBcov_info$pi+(1-ZINBcov_info$pi)*(1+ZINBcov_info$alpha*ZINBcov_info$mu)^(-1/ZINBcov_info$alpha)
# sum_stat$ZINBcov_mean <- (1-ZINBcov_info$pi)*ZINBcov_info$mu
# sum_stat$ZINBcov_var <- (1-ZINBcov_info$pi)*ZINBcov_info$mu*(1+ZINBcov_info$mu*(ZINBcov_info$pi+ZINBcov_info$alpha))

sum_stat$chooseAIC<-apply(sum_stat[,paste0(c('P','NB','ZIP','ZINB'),'_aic')],1,chooseAIC)
# sum_stat$chooseAIC2<-apply(sum_stat[,paste0(c('P','NB','ZIPcov','ZINBcov'),'_aic')],1,chooseAIC)
# sum_stat$chooseAIC3<-apply(sum_stat[,paste0(c('P','NB','ZIP','ZINB','ZIPcov','ZINBcov'),'_aic')],1,chooseAIC)
